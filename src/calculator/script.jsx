...
const Button = ({set, num, width, id}) => {
  num = num.toString()
  const style = {
    height: "3rem", background: "grey", cursor: "pointer",
    color: "white", display: "flex", "alignItems": "center",
    "justifyContent": "center", fontWeigt:800, fontSize: "2rem"
  }
  if (width === 2) {
    style.gridColumn = "span 2"
  }
  const handleClick = _ => set(num)
  return <div className="key" style={style} onClick={handleClick} id={id}>{num}</div>
}

const Grid = ({children}) => {
  const style = {display: "grid", gap: ".2rem", 
...
const App = () => {
  ...
  return <div id="calculator" style={{marginTop: "4rem"}}>
    ...
    <Grid>
      {
        buttons.map(el => {
          return <Button set={handleInput} num={el.symbol} width={el.width}
            id={el.id}/>
        })
      }
    </Grid>
